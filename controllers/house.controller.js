...
module.exports = class HouseController {
    ...
    static async fetchAllUserHousesExceptSelected(req, res) {
        try {
            const house = await House.find().populate("owners").find({ user: req.userData.userId, _id: { $ne: req.params.selectedHouse } });
    ...
    static async addUserToHouse(req, res) {
        ...
        if (house.creator == req.userData.userId) {
            House.updateOne({ _id: req.params.id }, { $addToSet: { owners: req.body.user } }).then(result => {
                res.status(200).json({ house });
            })
        } else {
